...
from logto.OidcCore import AccessTokenClaims, IdTokenClaims, OidcCore, TokenResponse, UserInfoResponse
...
class LogtoClient:
  async def getOidcCore(self) -> OidcCore:
    ...
    if self._oidcCore is None:
      self._oidcCore = OidcCore(await OidcCore.getProviderMetadata(
  ...
  def _setAccessToken(self, resource: str, accessToken: str, expiresIn: int) -> None:
    ...
    accessTokenMap.x[resource] = AccessToken(
      token=accessToken,
      expiresAt=int(time.time()) + expiresIn - 60, # 60 seconds earlier to avoid clock skew
    )
  ...
  async def _handleTokenResponse(self, resource: str, tokenResponse: TokenResponse) -> None:
    """
    ...
    if tokenResponse.id_token is not None:
      (await self.getOidcCore()).verifyIdToken(tokenResponse.id_token, self.config.appId)
    ...
    self._setAccessToken(resource, tokenResponse.access_token, tokenResponse.expires_in)

  async def _buildSignInUrl(self, redirectUri: str, codeChallenge: str, state: str, interactionMode: InteractionMode | None = None) -> str:
    appId, prompt, resources, scopes = self.config.appId, self.config.prompt, self.config.resources, self.config.scopes
    authorizationEndpoint = (await self.getOidcCore()).metadata.authorization_endpoint
    query = urllib.parse.urlencode(removeFalsyKeys({
      'client_id': appId,
  ...
  async def signIn(self, redirectUri: str, interactionMode: InteractionMode | None = None) -> str:
    """
    ...
    signInUrl = await self._buildSignInUrl(redirectUri, codeChallenge, state, interactionMode)

    self._setSignInSession(SignInSession(
  ...
  async def signOut(self, postLogoutRedirectUri: str | None = None) -> str:
    ...
    if endSessionEndpoint is None:
      raise LogtoException('End session endpoint not found in the provider metadata')
    
    return endSessionEndpoint + '?' + urllib.parse.urlencode(removeFalsyKeys({
  ...
  async def handleSignInCallback(self, callbackUri: str) -> None:
    ...
    if parsedCallbackUri.path != urllib.parse.urlparse(signInSession.redirectUri).path:
      raise LogtoException('The URI path does not match the redirect URI in the sign-in session')
